...
class ProfesoresModel extends Query{
    ...
    {
        $sql = "SELECT * FROM estudiante WHERE tipo = 'Profesor'";
        $res = $this->selectAll($sql);
        return $res;
    }
    public function insertarEstudiante($codigo, $dni, $nombre, $carrera, $direccion, $telefono)
    {
        $verificar = "SELECT * FROM estudiante WHERE tipo = 'Profesor' AND codigo = ?";
        $existe = $this->select($verificar, array($codigo));
        if (empty($existe)) {
            $query = "INSERT INTO estudiante(codigo,dni,nombre,carrera,direccion,telefono,tipo) VALUES (?,?,?,?,?,?,'Profesor')";
    ...
    {
        $sql = "SELECT * FROM estudiante WHERE tipo = 'Profesor' AND id = ?";
    ...
    {
        $sql = "SELECT id, codigo, nombre AS text FROM estudiante WHERE tipo = 'Profesor' AND (codigo LIKE ? OR nombre LIKE ?) AND estado = 1 LIMIT 10";
